...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.weight = 1 #yellow - plus 1
                        #green - plus 3
                        #red - plus 2
        self.image = pygame.transform.scale(pygame.image.load(f"r_assets/coin{self.weight}.png"), (40, 40))
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.image.load("r_assets/Enemy.png")
        self.rect = self.image.get_rect()
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.image.load("r_assets/Player.png")
        self.rect = self.image.get_rect()
...
class Game:
    ...
    def collectCoin(self):
        self.coins_points += self.coin.weight
        if self.coins_points > 3 and (self.coins_points / 4 > self.level):
            self.SPEED += 1
            self.level += 1
    ...
    def run(self):
        ...
        # road
        background = pygame.image.load("r_assets/AnimatedStreet.png")
        ...
        while True:
            ...
            coins_collected = font_small.render(f"Coins: {self.coins_points}", True, self.BLACK)
            self.DISPLAYSURF.blit(coins_collected, (SCREEN_WIDTH - 100, 10))

            level = font_small.render(f"Level: {self.level}", True, self.BLACK)
            self.DISPLAYSURF.blit(level, (SCREEN_WIDTH - 100, 30))
            ...
            if pygame.sprite.spritecollideany(self.player, self.enemies):
                pygame.mixer.Sound('r_assets/crash.wav').play()
                time.sleep(0.5)
